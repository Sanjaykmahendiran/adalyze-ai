...
const AuthLoginForm = ({ onSubmit, loading }: AuthLoginFormProps) => {
  ...
  const handleSendCode = async () => {
    ...
    try {
      const response = await fetch(`/api/forgot_otp?email=${encodeURIComponent(email)}`);
  ...
  const handleVerifyOTP = async () => {
    ...
    setResetLoading(true);
    try {
      const response = await fetch(`/api/forgot_verify_otp?email=${encodeURIComponent(email)}&otp=${encodeURIComponent(otp)}`);
      const result = await response.json();
  ...
  const handleUpdatePassword = async (data: ResetPasswordFormData) => {
    ...
    setResetLoading(true);
    try {
      const response = await fetch(`/api/updatepassword?email=${encodeURIComponent(verifiedEmail)}&password=${encodeURIComponent(data.newPassword)}&confirmpassword=${encodeURIComponent(data.confirmPassword)}`);
      const result = await response.json();
