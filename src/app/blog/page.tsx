  ...
  return (
    <>
      <div className="min-h-screen max-w-7xl mx-auto pt-10 pb-20">
        ...
        <div
          className={` flex flex-col items-center px-4 ${!isDashboard ? "py-36" : "pt-8 pb-8"
            }`}
        >
          {/* Heading Section */}
          <div
            className={`mb-8 sm:mb-16 w-full flex items-center ${isDashboard ? "justify-start" : "justify-center"
              }`}
          >
            {isDashboard && (
              <div
            ...
            <h2
              className={`text-2xl sm:text-4xl font-bold text-white ${isDashboard ? "text-left" : "text-center"
                }`}
            >
          ...
          {!loading && !error && (
            <div className="max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {blogPosts.map((post) => (
                <div
                  key={post.slug}
                  className="border border-white/10 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow bg-black"
                >
