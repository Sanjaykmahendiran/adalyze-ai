  ...
  return (
    <>
      <section className="w-full max-w-7xl mx-auto flex justify-center py-8 px-4">
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center justify-center bg-white rounded-3xl shadow-sm border border-[#171717] px-6 sm:px-8 md:px-10 py-6 gap-4 sm:gap-y-4 ">
          {partners.map((partner, index) => (
            ...
            >
              <Image
                ...
                width={140}
                height={60}
                className="object-contain max-w-[100px] sm:max-w-[140px] md:max-w-[150px] h-auto w-auto"
                loading="lazy"
              />
              {index < partners.length - 1 && (
                <div className="hidden md:block h-10 w-px bg-black/30 ml-6 md:ml-8" />
              )}
            </div>
          ))}
        </div>
