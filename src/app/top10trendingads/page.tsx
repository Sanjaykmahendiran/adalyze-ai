...
import { Trophy, ArrowLeft, Upload, TrendingUp } from "lucide-react"
    ...
    const fetchAdsData = async () => {
      try {
        ...
        const response = await fetch(
          "https://adalyzeai.xyz/App/tapi.php?gofor=trendingads"
        )
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <h1 className="text-lg sm:text-xl lg:text-2xl font-bold">
                Top 10 Trending Ads Wall
              </h1>
              <TrendingUp className="w-5 h-5 sm:w-6 sm:h-6 text-[#db4900]" />
            </div>
            <p className="text-sm sm:text-base text-white/80">
              Discover the most trending advertisements
            </p>
            {timeFrame && (
              <p className="text-xs sm:text-sm text-white/60 mt-1">
                {timeFrame} • {totalAdsConsidered} ads analyzed
              </p>
            )}
          </div>
        </header>
