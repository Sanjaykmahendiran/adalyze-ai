...
export default function UserLayout({ children,  userDetails }: UserLayoutProps) {
  ...
  return (
    <div className="flex relative">
      ...
      <div
        className={`hidden md:flex fixed inset-y-0 left-0 flex-col w-20 bg-card  z-50 print:hidden transition-all duration-300 ease-in-out`}
      >
        <div className="flex items-center justify-center h-16 px-2 ]">
          <div className={`flex items-center justify-center`}>
            <Link href={"/dashboard"}>
                <div className="w-14 h-14 rounded-full flex items-center justify-center text-foreground font-bold">
      ...
      <div className={`flex flex-col flex-1 h-screen md:ml-20 transition-all duration-300 ease-in-out`}>
        {/* TopNavbar */}
        <div className={`fixed top-0 md:left-20 left-0 right-0 flex items-center justify-between h-16 px-2 bg-card z-40 print:hidden transition-all duration-300 border-b border-border ease-in-out`}>
